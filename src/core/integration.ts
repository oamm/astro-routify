                                ...
                                load(id) {
                                    if (id === resolvedVirtualModuleId) {
                                        ...
                                        return `
                                            ...
                                            import { globalRegistry } from 'astro-routify';
                                            export const modules = import.meta.glob(${JSON.stringify(globPatterns)}, { eager: true });
                                            const register = (value) => {
                                                if (Array.isArray(value)) {
                                                    value.forEach(register);
                                                } else if (value && typeof value === 'object' &&
                                                    (value._routifyType === 'route' || value._routifyType === 'group')) {
                                                    globalRegistry.register(value);
                                                }
                                            };
                                            Object.values(modules).forEach((module) => {
                                                if (module && typeof module === 'object') Object.values(module).forEach(register);
                                            });
